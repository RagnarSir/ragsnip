...
def copy_image_to_clipboard(image_bytes: bytes, mime: str = "image/png") -> None:
    """Put raw image bytes on the X11 clipboard via xclip.

    xclip needs to keep running to serve the data to other apps until
    something else takes the clipboard. start_new_session detaches it
    from the parent process group so we don't need to wait().
    """
    require_tool("xclip")
    ...
    try:
        ...
        proc.kill()
        raise RuntimeError(f"xclip failed: {e}") from None
...
def run_gui(cfg: dict) -> int:
    ...
    def _render_card(entry: dict):
        ...
        def _copy_image_async():
            set_status(f"Downloading image…")

            def worker():
                try:
                    data = fetch_image_bytes(url)
                    copy_image_to_clipboard(data)
                    root.after(0, lambda: set_status(f"Copied image to clipboard ({len(data) // 1024} KB)"))
                except Exception as ex:
